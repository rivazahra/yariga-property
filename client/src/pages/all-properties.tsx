    ...
    <Box>
          ...
          <Box mb={2} mt={3} display="flex" width="84%" justifyContent="space-between" flexWrap="wrap">
            <Box display="flex" gap={2} flexWrap="wrap" mb={{ xs: '20px', sm: 0 }}>
              ...
              >
                ...
                {['Apartment', 'Villa', 'Farmhouse', 'Condo', 'Townhouse', 'Duplex', 'Studio', 'Hotel'].map((type) => (
      ...
      {allProperties.length > 0 && (
        <Box display="flex" gap={2} mt={3} flexWrap="wrap">
          <CustomButton title="Previous" handleClick={() => setCurrent((prev) => prev - 1)} backgroundColor="#475bef" color="#fcfcfc" disabled={!(current > 1)} />
          <Box color="#919191" display={{ xs: 'hidden', sm: 'flex' }} alignItems="center" gap="5px">
            Page{' '}
            <strong>
              {current} of {pageCount}
            </strong>
          </Box>
          <CustomButton title="Next" handleClick={() => setCurrent((prev) => prev + 1)} backgroundColor="#475bef" color="#fcfcfc" disabled={current === pageCount} />
          <Select
            sx={{
              fieldset: { borderColor: 'rgba(0,0,0,0.23)' },
              '& .MuiInputBase-root': {
